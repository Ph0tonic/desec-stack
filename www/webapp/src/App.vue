      ...
      <v-list dense>
        <v-list-item
          ...
          <v-list-item-content>
            <v-list-item-title>
              {{ item.text }}
              <v-icon :color="item.post_icon_color" class="text-grey-darken-2" size="small" v-if="item.post_icon">{{item.post_icon}}</v-icon>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
    ...
    <v-app-bar app class="bg-white">
      ...
      <div class="d-none d-md-block">
        <span class="mx-2" v-for="(item, key) in menu" :key="key">
          ...
          <v-icon :color="item.post_icon_color" class="ml-1 text-grey-darken-1" size="small" v-if="item.post_icon">{{item.post_icon}}</v-icon>
        </span>
      </div>
      ...
      <div class="bg-grey-darken-4 d-md-flex flex-row justify-space-between pa-6">
        ...
        <div>
          <p>
            ...
            <v-icon color="red" class="text-grey-darken-2" dense>{{ mdiHeart }}</v-icon>
          </p>
          <p>
